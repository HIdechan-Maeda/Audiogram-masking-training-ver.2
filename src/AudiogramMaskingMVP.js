...
function buildArtConfig(presetTargets, tympanogram, disorderName = null, casePattern = null) {
  ...
  const getEffectiveType = (type, ear) => {
    ...
    if ((casePattern === 'conductive' || casePattern === 'mixed') && type === 'As') {
...
export default function AudiogramMaskingMVP() {
  ...
  const [devPanelTab, setDevPanelTab] = useState('data'); // 'data', 'ai-test', 'disease-db', 'pattern-analysis'
...
以下のJSON形式で出力してください：
{
  ...
  const generateCaseDetails = async (generatedTargets, casePattern, generatedAge = null, patternAnalysis = null) => {
    ...
    if (patternAnalysis && patternAnalysis.possibleDisorders && patternAnalysis.possibleDisorders.length > 0) {
      ...
      if (Math.random() > 0.5) {
        ...
        if (casePattern === 'conductive' || casePattern === 'mixed') {
        ...
        } else if (casePattern === 'mixed') {
          // 混合性難聴の場合も、伝音成分があるためA型は不適切なことが多い
          if (tympType === 'A') {
            // 伝音成分が大きい場合はB型、As型、またはAd型にする
        ...
        // 所見をタイプに合わせて生成
        if (casePattern === 'conductive') {
          // 伝音性難聴の場合、ティンパ型に応じて中耳炎/離断の所見を補足
          const tympDesc = pattern.tympType === 'B' ? 'B型' : 
                          pattern.tympType === 'As' ? 'As型（コンプライアンス低）' : 
                          pattern.tympType === 'Ad' ? 'Ad型（コンプライアンス増大）' : 'B型';
          const middleEarNote = pattern.tympType === 'B'
            ? '（滲出性/急性中耳炎を示唆：鼓膜発赤/膨隆や液貯留所見）'
            : pattern.tympType === 'Ad'
            ? '（耳小骨離断を示唆）'
            : pattern.tympType === 'As'
            ? '（耳硬化症などを示唆）'
          ...
          // 混合性難聴の場合、タインパノグラム型に応じた所見
          const tympDesc = pattern.tympType === 'B' ? 'B型' : 
                          pattern.tympType === 'As' ? 'As型（コンプライアンス低）' : 
                          pattern.tympType === 'Ad' ? 'Ad型（コンプライアンス増大）' : 
                          'A型';
          findings = `鼓膜所見${pattern.tympType === 'B' || pattern.tympType === 'Ad' ? '異常あり' : '正常'}`;
        } else {
    ...
    // 伝音性難聴・混合性難聴の場合はタインパノグラム型を修正（疾患推定で設定されていない場合、または設定されていてもA型になっている場合）
    if (casePattern === 'conductive' || casePattern === 'mixed') {
      // 既に設定されているが、伝音性難聴なのにA型になっている場合は修正
      if (casePattern === 'conductive') {
        ...
        // 混合性難聴の場合、伝音成分があるため、A型の場合はB型、As型、またはAd型にする
        // 外傷エピソードがある場合はAd型を優先
        const traumaKeywords = ['殴られ', '耳掃除', 'ぶつかっ', '外傷', '頭部外傷', '転倒', 'ボールが', '打った'];
        const hasTrauma = traumaKeywords.some(keyword => 
          (chiefComplaint && chiefComplaint.includes(keyword)) || 
        ...
        if (!selectedDisorder || pattern.tympType === 'A') {
          if (hasTrauma) {
            ...
            const rand = Math.random();
            if (rand < 0.4) {
              pattern.tympType = 'B'; // 40%：B型
            } else if (rand < 0.7) {
              pattern.tympType = 'As'; // 30%：As型
            } else {
              pattern.tympType = 'Ad'; // 30%：Ad型
            }
          }
        } else if (hasTrauma && pattern.tympType !== 'Ad') {
          // 外傷エピソードがある場合はAd型に上書き
          pattern.tympType = 'Ad';
        }
      }
    }
  ...
  const analyzeHearingLossPattern = (targets, age = null) => {
    ...
    if (avgACOverall <= 20 && avgABGOverall <= 5) {
      pattern = 'normal';
    } else if (avgABGOverall > 20) {
      pattern = avgACOverall > 30 ? 'mixed' : 'conductive';
    } else {
  ...
  HEARING_DISORDERS.forEach(disorder => {
    ...
    if (c5Dip && (disorder.name.includes('騒音') || disorder.name.includes('音響'))) {
      ...
      // 低音障害（メニエール）
      if (pattern === 'sensorineural' && lowFreqAvg > midFreqAvg + 5 && disorder.name.includes('メニエール')) {
        score += 2;
      }
  ...
  const generateAICase = async () => {
    ...
    caseDetails.artConfig = buildArtConfig(adjustedTargets, caseDetails.tympanogram, selectedDisorderName, caseDetails.casePattern);
  ...
  return (
    <div className="w-full min-h-screen p-6 md:p-10 bg-gray-50 text-gray-900" ref={containerRef}>
      <div className="max-w-7xl mx-auto grid gap-6">
        ...
        {/* 開発者パネル */}
        {showDevPanel && (
          <div className="fixed right-4 top-4 bg-gray-900 text-white p-4 rounded-lg shadow-xl max-w-4xl w-[90vw] max-h-[85vh] overflow-hidden z-50 flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-lg font-bold">開発者パネル - AI症例生成 学習・改善ツール</h3>
                <p className="text-xs text-gray-400 mt-1">現在のシステムに影響を与えずにAI症例生成をテスト・改善できます</p>
              </div>
              <button
            ...
            <div className="flex gap-2 mb-4 border-b border-gray-700">
              <button
                onClick={() => setDevPanelTab('data')}
                className={`px-3 py-2 rounded-t ${devPanelTab === 'data' ? 'bg-gray-800 border-b-2 border-blue-500' : 'bg-gray-700 hover:bg-gray-650'}`}
              >
                データ表示
              </button>
              <button
                onClick={() => setDevPanelTab('ai-test')}
                className={`px-3 py-2 rounded-t ${devPanelTab === 'ai-test' ? 'bg-gray-800 border-b-2 border-blue-500' : 'bg-gray-700 hover:bg-gray-650'}`}
              >
                AI症例生成テスト
              </button>
              <button
                onClick={() => setDevPanelTab('disease-db')}
                className={`px-3 py-2 rounded-t ${devPanelTab === 'disease-db' ? 'bg-gray-800 border-b-2 border-blue-500' : 'bg-gray-700 hover:bg-gray-650'}`}
              >
                疾患データベース
              </button>
              <button
                onClick={() => setDevPanelTab('pattern-analysis')}
                className={`px-3 py-2 rounded-t ${devPanelTab === 'pattern-analysis' ? 'bg-gray-800 border-b-2 border-blue-500' : 'bg-gray-700 hover:bg-gray-650'}`}
              >
                パターン分析
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-4 text-sm">
                ...
                <>
                  <div className="bg-blue-900/30 border border-blue-500/50 rounded p-3 mb-4">
                    <p className="text-xs text-blue-200">
                      💡 <strong>使い方:</strong> このタブでは、現在読み込まれている症例のデータ構造を確認できます。JSON形式で表示されているので、データの整合性を確認したり、デバッグに活用できます。
                    </p>
                  </div>
                  <div>
                    <h4 className="font-semibold mb-2">現在の症例情報</h4>
                    <pre className="bg-gray-800 p-2 rounded overflow-auto text-xs max-h-40">
                      {JSON.stringify(currentCaseInfo, null, 2)}
                    </pre>
                  </div>
                  
                  <div>
                    <h4 className="font-semibold mb-2">ティンパノグラムデータ</h4>
                    <pre className="bg-gray-800 p-2 rounded overflow-auto text-xs max-h-32">
                      {JSON.stringify(currentCaseInfo?.tympanogram, null, 2)}
                    </pre>
                  </div>
                  
                  <div>
                    <h4 className="font-semibold mb-2">選択中のプリセット</h4>
                    <p>{selectedPreset}</p>
                  </div>
                  
                  <div>
                    <h4 className="font-semibold mb-2">AI生成症例詳細</h4>
                    <pre className="bg-gray-800 p-2 rounded overflow-auto text-xs max-h-40">
                      {JSON.stringify(customPresetDetails, null, 2)}
                    </pre>
                  </div>
                  
                  <div>
                    <h4 className="font-semibold mb-2">ターゲットデータ（最初の10件）</h4>
                    <pre className="bg-gray-800 p-2 rounded overflow-auto text-xs max-h-40">
                      {JSON.stringify(targets.slice(0, 10), null, 2)}
                    </pre>
                    <p className="text-xs text-gray-400 mt-1">全{targets.length}件</p>
                  </div>
                  
                  <div className="flex gap-2">
                    <button
                      onClick={() => {
                        console.log('Current Case Info:', currentCaseInfo);
                        console.log('Tympanogram Data:', currentCaseInfo?.tympanogram);
                        console.log('Targets:', targets);
                        console.log('Custom Preset Details:', customPresetDetails);
                        alert('コンソールにログを出力しました');
                      }}
                    ...
                    <button
                      onClick={() => {
                        if (currentCaseInfo?.tympanogram) {
                          const fixed = {
                            ...currentCaseInfo.tympanogram,
                            left: currentCaseInfo.tympanogram.left || currentCaseInfo.tympanogram.right || { peakPressure: 0, peakCompliance: 1.0, sigma: 60 },
                            right: currentCaseInfo.tympanogram.right || { peakPressure: 0, peakCompliance: 1.0, sigma: 60 }
                          };
                          setCurrentCaseInfo({ ...currentCaseInfo, tympanogram: fixed });
                          alert('ティンパノグラムデータを修正しました');
                        }
                      }}
              ...
              {devPanelTab === 'ai-test' && (
                <div className="space-y-4">
                  <div className="bg-gray-800 p-4 rounded">
                    <h4 className="font-semibold mb-2">AI症例生成テスト</h4>
                    <p className="text-xs text-gray-400 mb-4">
                      💡 <strong>使い方:</strong> ボタンをクリックすると、症例が生成されます。結果を確認して、アルゴリズムの動作を検証できます。生成された症例は画面には表示されず、開発パネル内でのみ確認できます。
                    </p>
                      ...
                      <button
                        onClick={async () => {
                          try {
                            const frequencies = [125, 250, 500, 1000, 2000, 4000, 8000];
                            const ears = ['R', 'L'];
                            const testDisorder = HEARING_DISORDERS[Math.floor(Math.random() * HEARING_DISORDERS.length)];
                            const testAge = Math.floor(Math.random() * (testDisorder.ageRange[1] - testDisorder.ageRange[0] + 1)) + testDisorder.ageRange[0];
                            const testTargets = generateDisorderSpecificAudiogram(testDisorder, testAge, frequencies, ears);
                            let patternAnalysis = analyzeHearingLossPattern(testTargets, testAge);
                            const caseDetails = await generateCaseDetails(testTargets, patternAnalysis.pattern, testAge, patternAnalysis);
                            // 職業性騒音の履歴がある場合は騒音性難聴を最優先に補正
                            const chronicNoiseKeywords = ['工場','建設','重機','製造ライン','騒音職場','長時間の音'];
                            const hasChronicNoise = chronicNoiseKeywords.some(k => (caseDetails.history||'').includes(k));
                            if (hasChronicNoise) {
                              const noise = HEARING_DISORDERS.find(d => d.name.includes('騒音'));
                              if (noise) {
                                const rest = (patternAnalysis.possibleDisorders||[]).filter(x => x.disorder.name !== noise.name);
                                patternAnalysis = { ...patternAnalysis, possibleDisorders: [{ disorder: noise, score: 999 }, ...rest] };
                              }
                            } else {
                              // 慢性騒音エピソードが無い場合は騒音性を減点して上位から外す
                              const updated = (patternAnalysis.possibleDisorders||[]).map(x => (
                                x.disorder.name.includes('騒音') ? { ...x, score: x.score - 10 } : x
                              )).sort((a,b)=>b.score-a.score);
                              patternAnalysis = { ...patternAnalysis, possibleDisorders: updated };
                            }
                            
                            setDevTestResults({
                              // 表示用の疾患は補正後（caseDetails.selectedDisorder）を優先
                              disorder: caseDetails.selectedDisorder || (patternAnalysis.possibleDisorders?.[0]?.disorder || testDisorder),
                              age: testAge,
                              targets: testTargets,
                              patternAnalysis,
                              caseDetails,
                            ...
                            console.error('AI症例生成テストエラー:', error);
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            if (caseType < 0.3) {
                              testAge = Math.floor(Math.random() * 46) + 5;
                              // 正常聴力パターン生成（簡略版）
                              ears.forEach(ear => {
                                frequencies.forEach(freq => {
                                  const normalRange = getNormalRangeForAge(testAge, freq);
                                  const dB = Math.round((Math.random() * (normalRange.max - normalRange.min) + normalRange.min) / 5) * 5;
                                  testTargets.push({ ear, transducer: 'AC', masked: false, freq, dB });
                                  testTargets.push({ ear, transducer: 'BC', masked: true, freq, dB: Math.max(0, dB - 5 + Math.floor(Math.random() * 11)) });
                                });
                              });
                            } else if (caseType < 0.6) {
                              testAge = Math.floor(Math.random() * 51) + 30;
                              // 感音性難聴パターン（簡略版）
                              ears.forEach(ear => {
                                const baseLoss = Math.round((Math.random() * 25 + 15) / 5) * 5;
                                frequencies.forEach(freq => {
                                  const normalThreshold = getNormalHearingThreshold(testAge, freq) || 5;
                                  let dB = normalThreshold + baseLoss;
                                  if (freq >= 2000) dB += Math.round((Math.random() * 20 + 10) / 5) * 5;
                                  testTargets.push({ ear, transducer: 'AC', masked: false, freq, dB: Math.round(dB / 5) * 5 });
                                  testTargets.push({ ear, transducer: 'BC', masked: true, freq, dB: Math.max(0, Math.round(dB / 5) * 5 - 5 + Math.floor(Math.random() * 11)) });
                                });
                              });
                            }
                            ...
                            const caseDetails = await generateCaseDetails(testTargets, patternAnalysis.pattern, testAge, patternAnalysis);
                            // 職業性騒音の履歴がある場合は騒音性難聴を最優先に補正
                            const chronicNoiseKeywords2 = ['工場','建設','重機','製造ライン','騒音職場','長時間の音'];
                            const hasChronicNoise2 = chronicNoiseKeywords2.some(k => (caseDetails.history||'').includes(k));
                            if (hasChronicNoise2) {
                              const noise = HEARING_DISORDERS.find(d => d.name.includes('騒音'));
                              if (noise) {
                                const rest = (patternAnalysis.possibleDisorders||[]).filter(x => x.disorder.name !== noise.name);
                            ...
                            setDevTestResults({
                              disorder: caseDetails.selectedDisorder || (patternAnalysis.possibleDisorders?.[0]?.disorder || null),
                              age: testAge,
                              targets: testTargets,
                              patternAnalysis,
                              caseDetails,
                    ...
                    {devTestResults && (
                      <div className="space-y-3 mt-4">
                        <div className="bg-gray-700 p-3 rounded">
                          <h5 className="font-semibold mb-2">生成情報</h5>
                          <div className="text-xs space-y-1">
                            <p>疾患: {devTestResults.disorder?.name || '汎用パターン'}</p>
                            <p>年齢: {devTestResults.age}歳</p>
                            <p>推定パターン: {devTestResults.patternAnalysis?.pattern}</p>
                            {devTestResults.patternAnalysis?.possibleDisorders && (
                              <div>
                                <p className="font-semibold mt-2">推定疾患（上位3位）:</p>
                                {devTestResults.patternAnalysis.possibleDisorders.slice(0, 3).map((d, i) => (
                                  <p key={i} className="pl-2">{(i+1)}. {d.disorder.name} (スコア: {d.score})</p>
                                ))}
                              </div>
                            )}
                          </div>
                        ...
                        <div className="bg-gray-700 p-3 rounded max-h-48 overflow-auto">
                          <h5 className="font-semibold mb-2">生成された症例詳細</h5>
                          <pre className="text-xs">
                            {JSON.stringify({
                              age: devTestResults.caseDetails.age,
                              gender: devTestResults.caseDetails.gender,
                              chiefComplaint: devTestResults.caseDetails.chiefComplaint,
                              history: devTestResults.caseDetails.history,
                              findings: devTestResults.caseDetails.findings,
                              tympanogram: devTestResults.caseDetails.tympanogram
                            }, null, 2)}
                          </pre>
                        </div>
                        
                        <div className="bg-gray-700 p-3 rounded max-h-48 overflow-auto">
                          <h5 className="font-semibold mb-2">オーディオグラムデータ（最初の10件）</h5>
                          <pre className="text-xs">
                            {JSON.stringify(devTestResults.targets.slice(0, 10), null, 2)}
                          </pre>
                          <p className="text-xs text-gray-400 mt-1">全{devTestResults.targets.length}件</p>
                        </div>
                        
                        <button
                          onClick={() => {
                            console.log('=== AI症例生成テスト結果 ===');
                            console.log('Disorder:', devTestResults.disorder);
                            console.log('Age:', devTestResults.age);
                            console.log('Pattern Analysis:', devTestResults.patternAnalysis);
                            console.log('Case Details:', devTestResults.caseDetails);
                            console.log('Targets:', devTestResults.targets);
                            alert('コンソールに詳細を出力しました');
                          }}
              ...
              {/* 疾患データベースタブ */}
              {devPanelTab === 'disease-db' && (
                <div className="space-y-4">
                  <div className="bg-gray-800 p-4 rounded">
                    <h4 className="font-semibold mb-2">疾患データベース ({HEARING_DISORDERS.length}種類)</h4>
                    <p className="text-xs text-gray-400 mb-3">
                      💡 <strong>使い方:</strong> AI症例生成で使用される疾患データベースを確認できます。各疾患の特徴（年齢範囲、性別傾向、オーディオグラムパターン等）を確認して、症例生成のロジックを理解できます。
                    </p>
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                      {HEARING_DISORDERS.map((disorder, index) => (
                        <div key={index} className="bg-gray-700 p-3 rounded">
                          <h5 className="font-semibold text-blue-400">{disorder.name}</h5>
                          <div className="text-xs mt-2 space-y-1">
                            <p><span className="text-gray-400">年齢範囲:</span> {disorder.ageRange[0]}-{disorder.ageRange[1]}歳</p>
                            <p><span className="text-gray-400">性別傾向:</span> {disorder.genderBias > 0.5 ? '女性' : disorder.genderBias < 0.5 ? '男性' : 'なし'}</p>
                            <p><span className="text-gray-400">オーディオグラム:</span> {disorder.audiogram}</p>
                            <p><span className="text-gray-400">ティンパノグラム:</span> {disorder.tympanometry}</p>
                            <p><span className="text-gray-400">アブミ骨筋反射:</span> {disorder.stapedial_reflex}</p>
                            <p><span className="text-gray-400">DPOAE:</span> {disorder.oae}</p>
                            {Array.isArray(disorder.episodes) && disorder.episodes.length > 0 && (
                              <div>
                                <p className="text-gray-400">エピソード:</p>
                                <ul className="list-disc list-inside space-y-0.5">
                                  {disorder.episodes.map((e, i) => (
                                    <li key={i}>{e}</li>
                                  ))}
                                </ul>
                              </div>
                            )}
                            {disorder.unilateral && <p className="text-yellow-400">一側性</p>}
                            {disorder.severity && <p className="text-yellow-400">重症度: {disorder.severity}</p>}
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              {/* パターン分析タブ */}
              {devPanelTab === 'pattern-analysis' && (
                <div className="space-y-4">
                  <div className="bg-gray-800 p-4 rounded">
                    <h4 className="font-semibold mb-2">パターン認識アルゴリズム分析</h4>
                    <p className="text-xs text-gray-400 mb-3">
                      💡 <strong>使い方:</strong> AI生成症例を読み込んだ後、このタブでパターン認識アルゴリズムがどのように難聴パターンを判定し、疾患を推定しているかを確認できます。まず「AI症例生成」ボタンで症例を生成してください。
                    </p>
                    {targets.length > 0 && currentCaseInfo?.caseId === 'AI生成' ? (
                      <div className="space-y-3">
                        {(() => {
                          try {
                            const analysis = analyzeHearingLossPattern(targets);
                            return (
                              <>
                                <div className="bg-gray-700 p-3 rounded">
                                  <h5 className="font-semibold mb-2">判定結果</h5>
                                  <div className="text-xs space-y-1">
                                    <p>パターン: <span className="text-blue-400">{analysis.pattern}</span></p>
                                    <p>平均AC: R={analysis.averages?.AC?.R?.toFixed(1)}dB, L={analysis.averages?.AC?.L?.toFixed(1)}dB</p>
                                    <p>平均BC: R={analysis.averages?.BC?.R?.toFixed(1)}dB, L={analysis.averages?.BC?.L?.toFixed(1)}dB</p>
                                    <p>平均気骨差: R={analysis.averages?.ABG?.R?.toFixed(1)}dB, L={analysis.averages?.ABG?.L?.toFixed(1)}dB</p>
                                    {analysis.hasC5Dip && <p className="text-yellow-400">C5 dip 検出</p>}
                                    {analysis.asymmetry && <p className="text-yellow-400">左右非対称</p>}
                                  </div>
                                </div>
                                
                                <div className="bg-gray-700 p-3 rounded max-h-64 overflow-auto">
                                  <h5 className="font-semibold mb-2">推定疾患（スコア順）</h5>
                                  <div className="text-xs space-y-1">
                                    {analysis.possibleDisorders?.slice(0, 5).map((d, i) => (
                                      <div key={i} className="border-l-2 border-blue-500 pl-2 mb-2">
                                        <p className="font-semibold">{i+1}. {d.disorder.name} (スコア: {d.score})</p>
                                        <p className="text-gray-400 text-xs">{d.disorder.epidemiology}</p>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                                
                                <pre className="bg-gray-700 p-3 rounded text-xs max-h-48 overflow-auto">
                                  {JSON.stringify(analysis, null, 2)}
                                </pre>
                              </>
                            );
                          } catch (error) {
                            return <p className="text-red-400">分析エラー: {error.message}</p>;
                          }
                        })()}
                      </div>
                    ) : (
                      <p className="text-gray-400">AI生成症例を読み込んでください</p>
                    )}
                  </div>
                </div>
              )}
        ...
        <button
          ...
          className="fixed right-4 bottom-20 bg-purple-600 text-white px-3 py-2 rounded-lg shadow-lg hover:bg-purple-700 z-40 text-sm"
          title="開発者パネル"
        >
          🔧 Dev
        </button>
