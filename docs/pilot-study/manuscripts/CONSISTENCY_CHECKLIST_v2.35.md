...
- 原稿: `manuscripts/manuscript_IgakuKensa_AudioScopeEDU_technical_validity_JAMT_v2.35.docx`
- 表: `manuscripts/Tables1-3.docx`
- 付録md/json: `manuscripts/appendix_IgakuKensa_verification_results.{md,json}`
- Supplement: `manuscripts/Supplement1_verification_v2.35.docx`
...
- [x] 手計算24例の条件表を電子付録に収載
- [x] T6a/T6c等の焦点条件（年齢・性別・程度・seed）を付録に明記
- [x] 伝音最小ABG下限の周波数別表を付録に追加
- [x] 骨導SO（NR規則のTH・判定・検証での扱い）を具体化
- [x] LCGおよびISO ±2SD→SD換算を付録（本文にもLCG一文）に記載
- [x] T8が3周波数平均の非減少である旨を明記
- [x] 要旨の「3条件各200件」を和英とも修正
- [ ] 図の固定行間（PDF切れ）は未対応（次版）
