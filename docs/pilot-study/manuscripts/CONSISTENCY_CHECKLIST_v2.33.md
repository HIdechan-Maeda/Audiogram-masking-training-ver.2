...
| 突発焦点T7 | 程度2・200件・中央値51.7・下限≥30 | 表2注／付録T7・§4注 |
| ムンプス下限 | meanAC≥70 かつ 一側差≥55（程度≥1） | II-3・表1・表2・付録 |
...
| Carhart深さ下限 | ≥5 dB（C5の10 dBと非対称・理由をII-3に記載） | II-3 |

## 全文検索で潰す語（残存0が目標）

- `12件` / `21項目` / `9項目`
- `≥25 dB`（突発規則としての下限）/ `25 dB未満となった例`
- `4 kHz気導閾値＞2 kHz`（旧C5定義）
- `軽度21.7` / `中等度38.3` / `重度58.3`（旧記述統計）
- `ムンプス` で「一側の加算・SO可」のみ（高度下限なし）
...
- 原稿: `manuscripts/manuscript_IgakuKensa_AudioScopeEDU_technical_validity_JAMT_v2.33.docx`
- 表: `manuscripts/Tables1-3.docx`
- 付録md/json: `manuscripts/appendix_IgakuKensa_verification_results.{md,json}`
- Supplement: `manuscripts/Supplement1_verification_v2.33.docx`
